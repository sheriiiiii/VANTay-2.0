...
// Skeleton component that matches the seat selection layout
function SeatSelectionSkeleton() {
  ...
  return (
    <div className="min-h-screen bg-blue-100 px-4 py-6">
      {/* Header with real title but skeleton driver info */}
      <div className="flex items-center mb-6">
        <button onClick={handleBack} className="mr-4" aria-label="Go back">
          <ArrowLeft className="h-6 w-6 text-gray-900" />
        </button>
        <div>
          <h1 className="text-2xl font-bold text-gray-700">Choose Your Seat</h1>
      ...
      <div className="max-w-xs mx-auto mb-8 px-4">
        {/* Top Row - Driver and Seat 01 Skeleton */}
        <div className="flex justify-between items-center gap-3 mb-4">
          <div className="bg-gray-200 rounded-xl p-4 flex-1 text-center min-h-[64px] flex items-center justify-center animate-pulse">
            <div className="w-12 h-4 bg-gray-300 rounded"></div>
          </div>
          <div className="w-20 h-16 mr-1 rounded-xl bg-gray-300 animate-pulse"></div>
        </div>

        {/* Passenger Seats Grid Skeleton */}
        <div className="grid grid-cols-3 gap-3 justify-items-center">
          {Array.from({ length: 12 }).map((_, i) => (
            <div key={i} className="w-20 h-16 rounded-xl bg-gray-300 animate-pulse"></div>
          ))}
        </div>
      </div>
          ...
          <p className="text-gray-600 text-sm">
            {seatData?.tripDetails.driverName && (
              <span className="flex items-center gap-1">
                ...
                Driver: {seatData.tripDetails.driverName}
              </span>
            )}
            {seatData?.tripDetails.arrivalTime && <span className="ml-2">• {seatData.tripDetails.arrivalTime}</span>}
          </p>
        </div>
      </div>
      ...
      <div className="max-w-xs mx-auto mb-8 px-4">
        ...
        <div className="flex justify-between items-center gap-3 mb-4">
          ...
          <button
            ...
            className={`w-20 h-16 mr-1 rounded-xl transition-colors flex items-center justify-center font-bold text-white text-sm ${
          ...
          {["02", "03", "04", "05", "06", "07", "08", "09", "10", "11", "12", "13"].map((seatNumber) => {
            ...
            return (
              <button
                ...
                className={`w-20 h-16 rounded-xl transition-colors flex items-center justify-center font-bold text-white text-sm ${getSeatColor(
                  status,
                )}`}
