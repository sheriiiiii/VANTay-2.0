...
export default function SeatSelection() {
  ...
  const handleSeatClick = async (seatNumber: string) => {
    ...
    if (!seat || seat.status === "occupied" || seat.status === "pending")
      return;
        ...
        <div>
          ...
          <p className="text-gray-600 text-sm">
            ...
            {seatData?.tripDetails.arrivalTime && (
              <span className="ml-2">• {seatData.tripDetails.arrivalTime}</span>
            )}
          </p>
        </div>
      </div>
        ...
        <div className="flex justify-between items-center gap-3 mb-4">
          ...
          <button
            ...
            className={`w-20 h-16 mr-1 rounded-xl transition-colors flex items-center justify-center font-bold text-white text-sm ${
              seats.find((s) => s.seatNumber === "01")?.status === "selected" ||
              selectedSeat === "01"
                ? "bg-blue-500 hover:bg-blue-600 cursor-pointer"
                : seats.find((s) => s.seatNumber === "01")?.status ===
                  "occupied"
            ...
            title="Seat 01"
            aria-label={`Seat 01 - ${
              seats.find((s) => s.seatNumber === "01")?.status || "available"
            }`}
          >
            01
          </button>
